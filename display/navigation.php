<!-- Navigation -->
		<div class="small-8 large-centered columns">
				<p></p>
				<div class="button-bar">
				<ul class="button-group">
			    	<li><a href="display/rawIngredients.php" class="small button">Raw Ingredients</a></li>
			    	<li><a href="display/recipes.php" class="small button">Recipes</a></li>
			    	<li><a href="display/batches.php" class="small button">Batches</a></li>
			    	<li><a href="display/packaging.php" class="small button">Packaging</a></li>
			    	<li><a href="display/finalProducts.php" class="small button">Final Products</a></li>
				  	</ul>
				</div>
		  	</div>
		</div>